...
class BuilderThread(QtCore.QThread):
    ...
    def run(self):
        try:
            with btypes.FileStream(self.filename[0],'wb',btypes.BIG_ENDIAN) as stream:
                kcl.pack(stream,self.triangles,self.max_triangles,self.min_width)
            with btypes.FileStream(os.path.splitext(self.filename[0])[0] + '.pa','wb',btypes.BIG_ENDIAN) as stream:
